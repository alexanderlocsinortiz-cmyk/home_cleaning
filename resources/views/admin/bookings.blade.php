                    ...
                    @foreach($activeBookings as $booking)
                        ...
                        <article class="grid gap-5 px-5 py-5 transition hover:bg-slate-50/70 xl:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.8fr)_minmax(300px,0.75fr)] xl:px-6">
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <div class="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Cleaner Assignment</div>
                                @if($requestedCleaner)
                                ...
                                <div class="mt-3 text-xs leading-5 text-slate-500">
                                    Assigned:
                                    <span class="font-bold text-slate-900">{{ $booking->staff?->display_name ?? 'Unassigned' }}</span>
                                </div>
                                <form action="{{ route('admin.bookings.status', $booking->id) }}" method="POST" class="mt-3 space-y-2">
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" name="status" value="{{ $booking->status }}">
                                    <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                                        <select name="staff_id" {{ $reviewLocked ? 'disabled' : '' }} class="min-w-0 rounded-xl border border-slate-300 px-3 py-2 text-xs focus:border-blue-500 focus:outline-hidden {{ $reviewLocked ? 'bg-slate-100 text-slate-400' : '' }}">
                                        ...
                                        <button type="submit" {{ $reviewLocked ? 'disabled' : '' }} class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-blue-700 text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-slate-300" title="Save staff assignment">
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                ...
                                @endif
                                <form action="{{ route('admin.bookings.status', $booking->id) }}" method="POST" class="mt-3 rounded-xl border border-slate-200 bg-white p-2">
                                    @csrf
                                    @method('PATCH')
                                    <div class="mb-1 px-1 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Status</div>
                                    <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                                        <select name="status" {{ $reviewLocked ? 'disabled' : '' }} class="min-w-0 rounded-lg border border-slate-300 px-2.5 py-2 text-xs focus:border-blue-500 focus:outline-hidden {{ $reviewLocked ? 'bg-slate-100 text-slate-400' : '' }}">
                                        ...
                                        <button type="submit" {{ $reviewLocked ? 'disabled' : '' }} class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-blue-700 text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-slate-300" title="Save status">
                                            <i class="fas fa-check text-xs"></i>
                                        </button>
                                    </div>
                                </form>
                                <form action="{{ route('admin.bookings.payment', $booking->id) }}" method="POST" class="mt-3 rounded-xl border border-slate-200 bg-white p-2">
                                    @csrf
                                    @method('PATCH')
                                    <div class="mb-1 px-1 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Payment</div>
                                    <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
                                        <select name="payment_status" class="min-w-0 rounded-lg border border-slate-300 px-2.5 py-2 text-xs focus:border-blue-500 focus:outline-hidden">
                                        ...
                                        <button type="submit" class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-blue-700 text-white transition hover:bg-blue-800" title="Save payment">
                                            <i class="fas fa-check text-xs"></i>
                                        </button>
                                    </div>
                                </form>
