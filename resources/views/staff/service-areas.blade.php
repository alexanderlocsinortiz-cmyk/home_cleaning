...
<section class="bg-gray-50 min-h-screen px-4 py-6 md:px-8 md:py-8">
    <div class="mb-6 text-center md:mb-8">
        ...
        <p class="text-base text-gray-600 md:text-lg">Interactive provider coverage map for Bukidnon</p>
        <p class="mx-auto mt-2 max-w-3xl text-sm text-amber-700">Purple pins show the municipality center only. Exact provider base locations remain private.</p>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-4 lg:min-h-[550px] lg:h-[calc(100vh-280px)]">
        <div class="order-2 flex flex-col gap-4 overflow-y-auto rounded-xl bg-white p-4 shadow-md lg:order-1 lg:col-span-1">
            ...
            <div class="rounded-lg border border-teal-100 bg-teal-50 p-3">
                ...
                <p class="mt-1 text-xs leading-5 text-teal-700">Purple pins appear where approved and activated providers cover the area.</p>
                <ul id="coverageAreaList" class="mt-2 space-y-1">
                    ...
                    <li class="flex items-center gap-2 p-2 rounded-lg cursor-pointer text-sm hover:bg-white justify-between" data-name="{{ $area['name'] }}">
                        <span class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full shrink-0 bg-teal-700"></span>{{ $area['name'] }}</span>
