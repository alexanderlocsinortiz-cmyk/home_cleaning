...
class AttendanceApiTest extends TestCase
{
    ...
    public function test_signed_device_request_cannot_be_replayed_with_the_same_nonce(): void
    {
        ...
        $device = $this->createDevice('ESP32-04');
        $payload = [
            'employee_code' => $staff->username,
            'punch_type' => 'in',
        ];
        $headers = $this->signedDeviceHeaders($device, 'POST', '/api/iot/attendance/punch', $payload);

        $this->withHeaders($headers)
            ->postJson('/api/iot/attendance/punch', $payload)
            ->assertOk();

        $this->withHeaders($headers)
            ->postJson('/api/iot/attendance/punch', $payload)
            ->assertUnauthorized()
            ->assertJson(['error' => 'Request has already been used.']);
    }
